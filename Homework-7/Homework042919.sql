...
SELECT upper(CONCAT(actor.first_name," ",
actor.last_name)) AS ConcatenatedString FROM actor;

-- 2a. You need to find the ID number, first name, and 
-- last name of an actor, of whom you know only the first 
...
SELECT last_name, first_name from actor WHERE last_name like '%GEN%';
...
-- be performing queries on a description, so create a column in 
-- the table `actor` named `description` and use the data type 
-- `BLOB` (Make sure to research the type `BLOB`, as the difference 
-- between it and `VARCHAR` are significant).
ALTER TABLE sakila.actor
ADD COLUMN description VARBINARY(8000) AFTER first_name;
...
AND language_id IN
  ...
  -- 7b. Use subqueries to display all actors who appear in the film 
  -- `Alone Trip`
